...
.d-manage-modal-control
	.fn-flex.flex-column(:style="{ width: '100%', paddingRight: '10px' }")
		c-select(
			...
			v-model="widget.events[eventType][activeIndex].eventClass",
			placeholder="事件类型")
			c-select-option(value="scene") 场景事件
			c-select-option(value="component") 组件事件
		c-select(
			v-if="isComponentClass",
			v-model="widget.events[eventType][activeIndex].ids",
			:style="{ marginBottom: '10px' }",
			placeholder="选择组件",
			multiple,
			filterable)
		...
		c-select(
			v-if="isComponentClass",
			:style="{ marginBottom: '10px' }",
			v-model="widget.events[eventType][activeIndex].triggerType",
			placeholder="事件类型")
		...
		c-control(label="更新加工",)
			template(slot="bottom")
				c-code(
					v-if="isComponentClass",
					v-model="widget.events[eventType][activeIndex].processBody",)
			template(slot="right")
				c-switch(v-model="widget.events[eventType][activeIndex].processEnable")
		c-select(
			v-if="isSceneClass",
			:style="{ marginBottom: '10px' }",
			v-model="widget.events[eventType][activeIndex].id",
			filterable,
			placeholder="目标场景")
		...
		c-select(
			...
			filterable,
			placeholder="场景过度动画",
			v-model="widget.events[eventType][activeIndex].animate")
...
export default {
	...
	computed: {
		...
		isComponentClass() {
			return this.widget.events[this.eventType][this.activeIndex].eventClass === 'component'
		},
		isSceneClass() {
			return this.widget.events[this.eventType][this.activeIndex].eventClass === 'scene'
		},
		isShowCustomEvents() {
			const ids = this.widget.events[this.eventType][this.activeIndex].ids
			return ids.length === 1
		},
		customEventsConfig() {
			const ids = this.widget.events[this.eventType][this.activeIndex].ids
